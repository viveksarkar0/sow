...
-- Clear existing terms data
DELETE FROM terms;

-- Insert new English terms content
INSERT INTO terms (locale, section, title, content, sort_order) VALUES
('en', 'registration-terms', 'Registration and Trial Terms', 
'<p><strong>BY clicking Invoice Now, you choose to register according to the information that you have typed in and the text on the registration page and the terms here, and you at the same time accept the terms here.</strong></p>
...
<p>If we do not receive such a notice from you before 14 days from registration, then the order, for natural reasons, cannot be changed. With registration it is meant the date and time when you did choose to press the button Invoice Now.</p>', 1),

('en', 'billing-pricing', 'Billing and Pricing', 
...
<p>Intermediation, as well as invoicing, may take place from K-Soft Sverige AB, Box 2826, 187 28 Täby. In the future, we may choose to cooperate with another company for e.g. intermediation and invoicing. However, the customer relationship is with us. The payment is made to the company from which the invoice comes.</p>', 2),

('en', 'subscription-renewal', 'Subscription and Renewal Terms', 
...
<p>License for the use of 123 Fakturera is of course sold in accordance with applicable laws.</p>', 3),

('en', 'data-privacy', 'Data Storage and Privacy', 
...
<p><strong>Have a great day!</strong></p>', 6);

-- Insert Swedish translations (simplified versions)
INSERT INTO terms (locale, section, title, content, sort_order) VALUES
('sv', 'registration-terms', 'Registrerings- och Provvillkor', 
'<p><strong>GENOM att klicka på Fakturera Nu väljer du att registrera dig enligt den information du har skrivit in och texten på registreringssidan och villkoren här, och du accepterar samtidigt villkoren här.</strong></p>
...
<p>Du har naturligtvis rätt att avsluta användningen av programmet utan några kostnader, genom att meddela oss via e-post innan 14 dagar från registreringen att du inte vill fortsätta med programmet, och då betalar du naturligtvis ingenting.</p>', 1),

('sv', 'billing-pricing', 'Fakturering och Prissättning', 
...
<p><strong>Alla priser är exklusive moms.</strong></p>', 2),

('sv', 'subscription-renewal', 'Prenumerations- och Förnyelsevillkor', 
'<p>Årsavgiften är på kontinuerlig basis, men om du inte vill fortsätta använda programmet behöver du bara säga upp dig trettio dagar före början av nästa ettårsperiod.</p>

<p>Introduktionserbjudandet (<strong>99 SEK per månad</strong>) gäller för årsavgiften Start för det första året. Efter det första året faktureras det ordinarie priset.</p>', 3),

('sv', 'data-privacy', 'Datalagring och Integritet', 
...
<p>Om du beställer som privatperson har du rätt att ångra dig enligt lag. Din information lagras så att vi kan hjälpa dig, etc.</p>', 4),

('sv', 'user-rights', 'Dina Rättigheter och Juridisk Information', 
'<p>Du har naturligtvis rätt att begära tillgång till, ändring och radering av den information vi har om dig. Du har också rätt att begära begränsning av databehandling och att invända mot databehandling samt rätten till dataportabilitet.</p>
...
<p><strong>Ha en bra dag!</strong></p>', 6);
